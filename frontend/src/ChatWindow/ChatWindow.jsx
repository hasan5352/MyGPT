...
function ChatWindow() {
	const {prompt, setPrompt, reply, setReply, currThreadId, setPrevChats, setNewChat, setAllThreads, setCurrThreadId, navigateToAuthPage} = useContext(MyContext);
	...
	const getReply = async () => {
		...
		setLoading(true); setNewChat(false);
		...
		try {
			...
			if (!currThreadId) {
				...
				// update UI
				setReply(body.aiResponse);
				setCurrThreadId(body.threadId);
				setPrevChats([ { role: 'user', content: prompt }, { role: 'robot', content: body.aiResponse } ]);
				// Prevent the reply-effect from appending the same messages again
				setPrompt('');
				// prepend new thread to list
				setAllThreads(prev => [{ id: body.threadId, title: body.title }, ...(prev || [])]);
			} else {
    ...
    useEffect(() => {
			const removeDropDown = (e) => {if (logoutBtnRef.current && !logoutBtnRef.current.contains(e.target)) setDropdownOpen(false)};
			document.addEventListener('mousedown', removeDropDown);

			if(prompt && reply) setPrevChats(prevChats => ([...prevChats, {role: "user", content: prompt},{role: "robot", content: reply}]));
    ...
    return (
			<div className="chatWindow">
				...
				<Chat></Chat>
				<ScaleLoader className="scale-loader" color="#fff" loading={loading} />
